...
experiments test four hypotheses. Throughout, ``\method{}'' denotes the full model
\emph{with the independence objective} (\cref{sec:method}); the same model
\emph{without} it is our primary ablation baseline (``\method{}\,$-$\,indep''),
isolating the effect of the disentanglement loss from the architecture. We instantiate
\method{} on a frozen Wan-VAE latent as a convenient, strong substrate --- a
proof-of-concept, not the claim; the factorization mechanism is substrate-agnostic.

\textbf{H1 (factorization).} \method{} splits video into a \emph{static} code (what
is in the scene) and a \emph{dynamic} code (how it moves): a cross-probe is
diagonal-dominant --- identity reads from the static code, motion from the dynamic
code, each near chance on the other (\cref{tab:q2_crossprobe}).
\textbf{H2 (semantics + usability).} The two codes are \emph{semantically meaningful
and independently usable} --- the static code linearly reads object identity, the
dynamic code reads position/velocity, and each is \emph{clean} of the other (the
independence objective drives identity out of the dynamic code, below an entangled
code at matched budget; \cref{tab:q2_disent}). This semantic purity is what makes the
codes usable downstream: motion-defined SSv2 reads from the dynamic code
(\cref{tab:ssv2}), appearance-defined UCF101 and robot control from the static and
dynamic codes (\cref{tab:ucf101,tab:q3_action}).
\textbf{H3 (efficiency).} The factorization is \emph{compact}: at $96$--$352$ floats it
carries the usable semantic content --- more per float than any other compression of
the same substrate (PCA, mean-pool, random, full latent), by rate curve, label
efficiency, and decodability (\cref{tab:q1_semantic,tab:q1_labeleff,tab:q7_decode}).
\textbf{H4 (reconstruction, secondary).} We tested whether a cleaner split, by
removing redundancy, frees capacity and aids reconstruction where capacity binds. It
does \emph{not}: the independence objective is a small reconstruction \emph{tradeoff}
(recon-neutral at full rate on CLEVRER, but a modest cost at low rate and on SSv2), not
a gain --- the constraint removes a useful degree of freedom faster than it frees one.
We report this honestly; it is why we treat disentanglement as buying \emph{separation
and interpretability at a small efficiency cost}, not free improvement. Separately, the
spatial dynamics code is what makes real-video reconstruction viable at all (the
$+4.4$\,dB finding, \cref{tab:q4_recon}).

\textbf{Where \method{} wins.} In one line: \method{} \emph{wins on disentanglement and
efficiency while reconstruction stays competitive}. It has the lowest identity leakage
of any model, including a matched-rate entangled autoencoder (\cref{tab:q2_disent}) --- a
separation that entangled representations (raw VAE, DINOv2/VideoMAE, predictive VAEs)
cannot even express; it retains more reconstructable information per float than any other
compression of the same substrate (\cref{tab:q7_decode}) and is the most label-efficient
(\cref{tab:q1_labeleff}); and its reconstruction ties the un-regularized model rather than
degrading. We do \emph{not} claim to beat web-pretrained encoders on raw accuracy or codecs
on fidelity, and say so; the wins are on the axes the factorization is built for.
...
\begin{table}[t]
\centering\small
\caption{\textbf{H2 -- clean separation: identity leakage into the dynamic code}
(CLEVRER, colour is the discriminative attribute; chance $0.50$). The independence
objective is what makes the separation clean: it drives object identity \emph{out} of
$\zd$ (colour leakage $\to$ chance) while $\zs$ keeps it, widening the static/dynamic
gap over the no-loss ablation. This is the effect a matched-rate entangled code cannot
achieve: \method{}(\,$+$\,indep) has the \emph{lowest} identity leakage of the three
(at chance), and the widest static/dynamic gap. Full $200$-epoch CLEVRER models [C].}
\label{tab:q2_disent}
\begin{tabular}{lccc}
\toprule
Model & $\zs$ identity $\uparrow$ & $\zd$ leakage $\downarrow$ & gap $\uparrow$ \\
\midrule
\textbf{\method{} (\,$+$\,indep)}          & 0.646 & \textbf{0.507} & \textbf{0.139} \\
\method{}\,$-$\,indep (no loss, ablation)  & 0.679 & 0.564          & 0.115 \\
