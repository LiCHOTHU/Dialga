...
\textbf{H4 (reconstruction, secondary).} Because a cleaner split removes redundancy,
disentanglement \emph{may} free capacity and aid reconstruction where capacity binds
(low rate, real video); we report this as an observation, not a core claim
(\cref{tab:q4_recon}). The spatial dynamics code is what makes real-video
reconstruction viable at all (the $+4.4$\,dB finding, \cref{tab:q4_recon}).
...
achieve. \pmark{}\,=\,preliminary ($40$-epoch, $600$-video A/B; full-model numbers land
from \texttt{clevrer\_disent} and replace these).}
\label{tab:q2_disent}
\begin{tabular}{lccc}
\toprule
Model & $\zs$ identity $\uparrow$ & $\zd$ leakage $\downarrow$ & gap $\uparrow$ \\
\midrule
\textbf{\method{} (\,$+$\,indep)}          & 0.605\,\pmark & \textbf{0.499}\,\pmark & \textbf{0.106} \\
\method{}\,$-$\,indep (no loss, ablation)  & 0.640\,\pmark & 0.548\,\pmark          & 0.092 \\
Entangled AE (shared trunk)                & \tbd          & \tbd                   & \tbd \\
\bottomrule
\end{tabular}
\end{table}
