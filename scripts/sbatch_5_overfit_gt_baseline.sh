...
#SBATCH --output=/storage/project/r-agarg35-0/lwang831/outputs/dialga/slurm/dlg_5_gt_%j.out
#SBATCH --error=/storage/project/r-agarg35-0/lwang831/outputs/dialga/slurm/dlg_5_gt_%j.err

# Sanity-check baseline: GT collision-mask teacher. Establishes the ceiling
# for the head pipeline. If F1 << 0.5 here, head extraction is the bottleneck;
# any "good" teacher won't help. If F1 >> 0.5, the head works fine and the
# task is just to design a teacher that approximates GT well.
...
RUN_NAME=${RUN_NAME:-slot_5_gt_$(date +%m%d_%H%M)}
