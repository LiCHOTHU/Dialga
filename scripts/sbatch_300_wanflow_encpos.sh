#!/bin/bash
#SBATCH --job-name=dlg_300_wf_encpos
...
#SBATCH --output=/storage/project/r-agarg35-0/lwang831/outputs/dialga/slurm/dlg_300_wf_encpos_%j.out
#SBATCH --error=/storage/project/r-agarg35-0/lwang831/outputs/dialga/slurm/dlg_300_wf_encpos_%j.err

# Wan-flow decoder, encoder-pred q (the production-pipeline path) at scale.
...
RUN_NAME=${RUN_NAME:-wanflow_300_encpos_${GROUP}}
...
echo "=========================================="
echo " Wan-flow decoder @ 300 videos (encoder-pred q)"
echo "=========================================="
...
"${PYTHON}" -u scripts/overfit_wan_flow.py \
    ...
    --wandb_name "${RUN_NAME}" \
    --wandb_group "${GROUP}" \
    --wandb_tags "scale_300,wanflow,enc_pos,production"

echo "[$(date)] Wan-flow enc-q done."
