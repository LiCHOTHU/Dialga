...
#SBATCH --output=/storage/project/r-agarg35-0/lwang831/outputs/dialga/slurm/dlg_5_m2_%j.out
#SBATCH --error=/storage/project/r-agarg35-0/lwang831/outputs/dialga/slurm/dlg_5_m2_%j.err

# Motion teacher v2: GT-q mask (stable across epochs) + hard binarization
# + pos_weight=50 (to push sigmoid > 0.5 at sparse positives).
# Soft labels with sharpness=8 was producing mid-range ~0.4 mass on every
# slot-frame; hard_binarize=true converts to clean {0,1} targets.
...
RUN_NAME=${RUN_NAME:-slot_5_motion_v2_$(date +%m%d_%H%M)}
...
"${PYTHON}" scripts/train_slot.py \
    ...
    training.event_label_dilation=3 training.event_supervision=motion \
    training.self_event_z_thresh=2.0 training.self_event_sharpness=8.0 \
